...
const base::FilePath::CharType kBrowserProcessExecutableName[] = "chromium";
...
bool IsChromeProcess(pid_t pid) {
  ...
  return (!other_chrome_path.empty() &&
          other_chrome_path.BaseName() ==
              base::FilePath(kBrowserProcessExecutableName));
}
...
class ProcessSingleton::LinuxWatcher
    : public base::RefCountedThreadSafe<
          ProcessSingleton::LinuxWatcher,
          lynxtron::GlobalThread::DeleteOnIOThread> {
 public:
   ...
   public:
    SocketReader(ProcessSingleton::LinuxWatcher* parent,
        ...
        : parent_(parent),
          ...
          fd_(fd),
          bytes_read_(0) {
      DCHECK_CURRENTLY_ON(lynxtron::GlobalThread::IO);
      // Wait for reads.
 ...
 private:
  friend struct lynxtron::GlobalThread::DeleteOnThread<
      lynxtron::GlobalThread::IO>;
  friend class base::DeleteHelper<ProcessSingleton::LinuxWatcher>;

  ~LinuxWatcher() { DCHECK_CURRENTLY_ON(lynxtron::GlobalThread::IO); }
...
void ProcessSingleton::LinuxWatcher::SocketReader::FinishWithACK(
  ...
  lynxtron::GlobalThread::GetIOThreadTaskRunner()->PostTask(
      FROM_HERE,
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcessWithTimeout(
  ...
  for (int retries = 0; retries <= retry_attempts; ++retries) {
    ...
    if (hostname != GetHostName() && !IsChromeProcess(pid)) {
...
bool ProcessSingleton::Create() {
  ...
  base::FilePath symlink_content(
      base::StringPrintf("%s%c%u", GetHostName().c_str(),
...
void ProcessSingleton::StartWatching() {
  ...
  // DCHECK(
  //     lynxtron::GlobalThread::IsThreadInitialized(lynxtron::GlobalThread::IO));
  lynxtron::GetIOThreadTaskRunner()->PostTask(
...
bool ProcessSingleton::KillProcessByLockPath(bool is_connected_to_socket) {
  ...
  if (!hostname.empty() && hostname != GetHostName() &&
